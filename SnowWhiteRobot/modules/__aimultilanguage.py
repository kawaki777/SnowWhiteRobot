...
#    This programme is a part of SnowWhite (TG bot) project
#
...
@snowWhite.on_message(filters.command("chatbot") & ~filters.edited & ~filters.bot)
@admins_only
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("snowWhite AI Already Activated In This Chat")
            return
        await lel.edit(
            f"snowWhite AI Successfully Added For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("snowWhite", "Aco")
        test = test.replace("snowWhite", "Aco")
        querystring = {
            "bid": "178",
        ...
        result = result.replace("Aco", "snowWhite")
        result = result.replace("<a href=\\", "<a href =")
        result = result.replace("<\/a>", "</a>")
        pro = result
        try:
            await snowWhite.send_chat_action(message.chat.id, "typing")
...
@snowWhite.on_message(filters.text & filters.private & filters.reply & ~filters.bot)
...
@snowWhite.on_message(
    filters.regex("snowWhite|SnowWhite|The_Ghosht_Hunter|hello|hi")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
